<template>
  ...
  <blank-bar pbgcolor="bg-secondary" ptext="apps"></blank-bar>
  <tab-bar
    :ptab="tab"
        ...
        <div v-for="element in tabOptions" :key="element.name">
          <div v-if="tab == element.name">
            <list-item
              ...
              pidentifier="name"
              pname="name"
              pdetaillink="setup-apps-detail"
              :ploaddata="true"
              pobject="apps"
              porderby="name"
              :pshowaddbutton="true"
              paddbuttonurl="setup-apps-detail"
...
export default {
  ...
  setup() {
    ...
    const userid = ref('');
    const useremail = ref('');
    const tab = ref('all apps');
    const tabOptions = ref([{ name: 'all apps' }]);
