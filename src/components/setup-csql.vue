<template>
  ...
  <blank-bar pbgcolor="bg-secondary" ptext="csql"></blank-bar>
  <tab-bar
    :ptab="tab"
        ...
        <div v-for="element in tabOptions" :key="element.name">
          <div v-if="tab == element.name">
            <list-item
              ...
              pidentifier="name"
              pname="name"
              pdetaillink="setup-csql-detail"
              :ploaddata="true"
              pobject="csql"
              porderby="name"
              :pshowaddbutton="true"
              paddbuttonurl="setup-csql-detail"
...
export default {
  ...
  setup() {
    ...
    const userid = ref('');
    const useremail = ref('');
    const tab = ref('all csql');
    const tabOptions = ref([{ name: 'all csql' }]);
